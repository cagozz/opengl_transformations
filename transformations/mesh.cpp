...
void Mesh::rotateArbitaryAxis(Vec3f p1, Vec3f p2, float alpha)
{
	...
	else
	{
		if (p1.y() == p2.y())
		{
			...
			Mat4f rotate_around_x(1);
			rotate_around_x[5] = rotate_around_x[10] = cos(alpha);;
			rotate_around_x[9] = sin(alpha); rotate_around_x[6] = -sin(alpha);

			Mat4f undo_translate(1);
			undo_translate[3] = 0; undo_translate[7] = p1.y(); undo_translate[11] = p1.z();
...
void Mesh::reflect(Mesh& plane)
{
	...
	{
		Mat4f translate(1);
		translate[3] = -point.x(); translate[7] = -point.y(); translate[11] = -point.z();
		...
		Mat4f rotate_around_y(1);
		rotate_around_y[0] = rotate_around_y[10] = d / normal.length();
		rotate_around_y[2] = -a / normal.length(); rotate_around_y[8] = a / normal.length();

		Mat4f reflect(1);
		reflect[10] = -1;

		Mat4f undo_y(1);
		undo_y[0] = undo_y[10] = d / normal.length();
		undo_y[2] = a / normal.length(); undo_y[8] = -a / normal.length();
		...
		Mat4f undo_translate(1);
		undo_translate[3] = point.x(); undo_translate[7] = point.y(); undo_translate[11] = point.z();

		Mat4f result =  undo_translate * undo_x * undo_y * reflect * rotate_around_y * rotate_around_x * translate;
		model = result * model;
	}
	else
	{
